...
ADAPTER = r'''"""PR-F research-only SportyBet upcoming-event reconciliation adapter.
...
EXPECTED_CONTRACT_SHA256 = "__PRF_UPCOMING_CONTRACT_SHA256__"
...
(ROOT / 'domain/current_shadow_sportybet_upcoming_reconciliation.py').write_text(ADAPTER, encoding='utf-8')
(ROOT / 'tests/test_current_shadow_sportybet_upcoming_reconciliation.py').write_text(TEST, encoding='utf-8')
...
# Contract hash is computed from the final source, then pinned without touching the shared PR251 hash.
import importlib.util
import sys
spec = importlib.util.spec_from_file_location(
    'domain.current_shadow_sportybet_upcoming_reconciliation',
    ROOT / 'domain/current_shadow_sportybet_upcoming_reconciliation.py',
)
module = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = module
spec.loader.exec_module(module)
actual = module.calculate_contract_sha256()
p = ROOT / 'domain/current_shadow_sportybet_upcoming_reconciliation.py'
text = p.read_text(encoding='utf-8')
if text.count('__PRF_UPCOMING_CONTRACT_SHA256__') != 1:
    raise SystemExit('adapter contract placeholder count drifted')
p.write_text(text.replace('__PRF_UPCOMING_CONTRACT_SHA256__', actual), encoding='utf-8')

for path in (
    ROOT / '.github/prf_apply_upcoming_mirror.py',
    ROOT / '.github/workflows/prf-one-shot-upcoming-mirror.yml',
):
    if path.exists():
        path.unlink()
