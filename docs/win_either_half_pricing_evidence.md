...
Coverage has an explicitly labelled `ALL_ROLES_DESCRIPTIVE` total and a
separate breakdown for every role. No unlabeled mixed-role analysis is emitted.
...
Provider mappings are a separate JSON list. Each mapping binds the exact
...
fixture identifier. They are research protocol inputs, not selected from TEST
performance.
...
A complete market snapshot contains exactly one `YES` and one `NO` quote. Both
...
across bookmakers, snapshots, or observation times.
...
The method is recorded as `multiplicative_normalization`. Numerical output is
canonicalized to 12 decimal places before serialization. Stage 5A does not
compare alternative de-vig methods.
