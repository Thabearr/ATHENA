# FotMob ordinary-FT finished-score source-history completeness protocol

## Purpose

This boundary pre-registers the next source-history evidence contract after PR #98 registered the separate reviewed FotMob ordinary-FT finished-score source capability.

The derived source key is:

`fotmob_data_matches_reviewed_ordinary_ft_finished_score`

It now has reviewed source-reported finished-score semantics for fixtures that pass the exact ordinary-FT gate, but its `historical_coverage` remains `UNKNOWN`. The new score capability therefore must not be treated as proof that ATHENA has a complete historical corpus suitable for successor replay.
...
The registered derived source is frozen for this boundary as:
...
The parent `fotmob_data_matches_reviewed_catalog` remains unchanged with `full_time_score = NOT_CAPTURED`, reliable fixture identity `CONFIRMED`, and historical coverage `UNKNOWN`.

## Score scope remains narrow

`CONFIRMED` means only source-reported finished score for the exact reviewed ordinary-FT gate. It does not establish regulation-time, extra-time, penalty-score, bookmaker-settlement, or global status-reason semantics.
...
The protocol is pre-registered but not executed. Its current pre-execution disposition is:

`BLOCKED_HISTORICAL_COVERAGE_UNPROVEN`

No source-history adapter is approved, completeness is not proven, PR #80 constructor input is not authorized, and no successor/model/probability/pricing/selection/production/BET authority changes.
