"""Prepare the exact reviewed Saturday 2026-08-22 Fixture Catalog and admission decision.

This is an offline evidence-replay operator boundary. It consumes the exact PR #199
FotMob source artifact plus the explicit PR #201 fixture-review ledger, rebuilds the
reviewed catalog through the existing PR #43 workflow, checks the generated bytes,
and emits the exact PR #45/PR #165 catalog-admission decision candidate for a later
separate explicit byte review.
...
def _materialize_exact_source_capture(
    *,
    ...
    source_fixture = source_root / "fixture"
...
def execute(
    *,
    ...
    if generated.summary != checked.summary | {"mode": "GENERATE"}:
        # The summaries should differ only in the explicit mode field.
        generated_without_mode = dict(generated.summary)
        checked_without_mode = dict(checked.summary)
        generated_without_mode.pop("mode", None)
        checked_without_mode.pop("mode", None)
        if generated_without_mode != checked_without_mode:
            raise SaturdayReviewedFixtureCatalogPreparationError(
                "generated and checked catalog replay summaries differ"
            )

    summary = generated.summary
    if summary["candidate_bundle_sha256"] != EXPECTED_CANDIDATE_BUNDLE_SHA256:
        raise SaturdayReviewedFixtureCatalogPreparationError(
            "rebuilt candidate bundle differs from PR199/PR201 boundary"
        )
    if summary["candidate_count"] != EXPECTED_SOURCE_CANDIDATE_COUNT:
        raise SaturdayReviewedFixtureCatalogPreparationError(
            "rebuilt candidate count differs from 670"
        )
    if summary["decision_count"] != EXPECTED_APPROVED_FIXTURE_COUNT:
        raise SaturdayReviewedFixtureCatalogPreparationError(
            "review decision count differs from 50"
        )
    if summary["approved_count"] != EXPECTED_APPROVED_FIXTURE_COUNT:
        raise SaturdayReviewedFixtureCatalogPreparationError(
            "approved catalog identity count differs from 50"
        )
    if summary["rejected_count"] != 0:
        raise SaturdayReviewedFixtureCatalogPreparationError(
            "reviewed Saturday catalog unexpectedly contains rejections"
        )
    if summary["unreviewed_count"] != EXPECTED_UNREVIEWED_FIXTURE_COUNT:
        raise SaturdayReviewedFixtureCatalogPreparationError(
            "unreviewed candidate count differs from 620"
        )
    if summary["blocked_candidate_count"] != 0:
        raise SaturdayReviewedFixtureCatalogPreparationError(
            "reviewed Saturday catalog unexpectedly contains review blockers"
        )
    if summary["fixture_count"] != EXPECTED_APPROVED_FIXTURE_COUNT:
        raise SaturdayReviewedFixtureCatalogPreparationError(
            "compiled Fixture Catalog does not contain exactly 50 fixtures"
        )
    ...
    forbidden_true = {
        ...
        if key != "fixture_catalog_compile_performed" and value is True
    }
    if forbidden_true != {"fixture_catalog_write_performed"}:
        raise SaturdayReviewedFixtureCatalogPreparationError(
    ...
    actual_ids = {int(item.source_fixture_identifier) for item in generated.handoff.catalog_inputs}
    ...
    _write_once(output / "reviewed-fixture-catalog-prepare-receipt.json", receipt_bytes)
    return receipt
