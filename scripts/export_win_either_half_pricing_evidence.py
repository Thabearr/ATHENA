...
def load_provider_mappings(path: Path) -> tuple[dict, dict]:
...
def load_decisions(path: Path) -> tuple[dict[str, Any], dict]:
    ...
    for row in value:
        ...
        decisions[fixture] = row.get("decision_at")
    ...
    *,
    ...
    role_counts = {
        role: {
            "accepted_quotes": sum(
                1
                for result in quote_results
                if result.record is not None and result.record.evaluation_role == role
            ),
            "accepted_snapshots": sum(
                1
                for result in snapshot_results
                if result.snapshot is not None
                and result.snapshot.evaluation_role == role
                and result.status is EvidenceStatus.ACCEPTED
            ),
            "selected_latest_snapshots": sum(
                1
                for result in snapshot_results
                if result.snapshot is not None
                and result.snapshot.evaluation_role == role
    ...
    return {
        ...
        "coverage": {
            ...
            "selected_latest_snapshots": sum(
                result.selected for result in snapshot_results
            ),
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        mappings, mapping_identity = load_provider_mappings(args.provider_mappings)
        decisions, decision_identity = load_decisions(args.decisions)
