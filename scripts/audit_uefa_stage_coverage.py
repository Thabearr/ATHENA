...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--training-view",
        type=Path,
        help="Optional frozen Goal/Score training-view SQLite to audit by exact match-key join.",
    )
    parser.add_argument(
        "--output-json",
        type=Path,
        help="Optional path for the canonical JSON report.",
    )
...
def main() -> int:
    ...
    report["uefa_parent_identity_sha256"] = parent_sha
    report["uefa_stage_registry_sha256"] = registry_sha
    report["uefa_stage_contract_sha256"] = contract_sha
    text = json.dumps(
        report,
        sort_keys=True,
        separators=(",", ":"),
