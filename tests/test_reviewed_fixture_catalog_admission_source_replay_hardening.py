...
def test_hash_shaped_forged_decision_lineage_cannot_cross_source_replay(
    tmp_path: Path,
) -> None:
    handoff, result, decision = _decision(tmp_path)
    forged_native = dataclasses.replace(
        decision.decision,
        handoff_sha256="f" * 64,
    )
...
def test_stored_admission_tampering_fails_source_aware_verification(
    tmp_path: Path,
) -> None:
    handoff, result, decision = _decision(tmp_path)
    ...
    with pytest.raises(
        ...
        match="stale|tampered|exact source-replayed",
    ):
        replay.verify_source_replayed_admission_directory(
            directory,
            handoff=handoff,
            fixture_catalog_result=result,
            repository_root=tmp_path,
...
def test_stored_decision_tampering_fails_before_admission_authority(
    tmp_path: Path,
) -> None:
    handoff, result, decision = _decision(tmp_path)
    directory, _ = replay.store_source_replayed_admission(
...
def test_unexpected_entry_fails_closed_and_is_not_deleted(
    tmp_path: Path,
) -> None:
    handoff, result, decision = _decision(tmp_path)
    directory, _ = replay.store_source_replayed_admission(
...
def test_mkdir_race_never_deletes_competing_writer_directory(
    tmp_path: Path,
...
def test_alternate_output_root_and_traversal_fail_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        replay.ReviewedFixtureCatalogAdmissionSourceReplayError,
        match="reviewed exact source-replay root",
    ):
        replay.store_source_replayed_admission(
            handoff=handoff,
            fixture_catalog_result=result,
...
def test_rejected_and_admitted_decisions_have_distinct_artifact_identity(
    tmp_path: Path,
) -> None:
    ...
    assert (
        sha256_reviewed_fixture_catalog_admission(admitted)
        != sha256_reviewed_fixture_catalog_admission(rejected)
    )
