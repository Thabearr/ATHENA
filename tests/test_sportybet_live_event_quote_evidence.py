...
from tests import test_sportybet_reviewed_canonical_market_mapping as mapping_fixture
...
def _reviewed_mapping(monkeypatch):
    row = mapping_fixture._selection(
        market="18",
        name="Total Goals",
        spec="total=2.5",
        outcome="O",
    ...
    decision = mapping_fixture._decision(
        row,
        market=MarketId.TOTAL_GOALS,
        outcome=OutcomeId.OVER,
        line=2.5,
    )
    return mapping_fixture._build(monkeypatch, row, decision)
