...
def test_classification_cannot_predate_policy_and_must_remain_pre_kickoff() -> None:
    observed, policy_reviewed, deadline, kickoff = _times()

    assert is_within_reviewed_freshness_window(
        classified_at=policy_reviewed,
        observed_at=observed,
        policy_reviewed_at=policy_reviewed,
        fresh_until=deadline,
        kickoff=kickoff,
    ...
    for invalid in (
        policy_reviewed - datetime.timedelta(microseconds=1),
        kickoff,
...
def test_invalid_policy_deadline_or_observation_chronology_fails_closed() -> None:
    observed, policy_reviewed, _, kickoff = _times()

    for invalid_policy_time in (
        ...
        with pytest.raises(
            ...
            is_within_reviewed_freshness_window(
                classified_at=policy_reviewed,
                observed_at=observed,
                policy_reviewed_at=invalid_policy_time,
                fresh_until=policy_reviewed,
                kickoff=kickoff,
            )
