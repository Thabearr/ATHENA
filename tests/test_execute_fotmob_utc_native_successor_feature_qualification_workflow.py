...
def test_execution_is_bound_to_exact_merged_control_pr_and_owner_command() -> None:
    ...
    required = (
        ...
        "const controlPr = 138;",
        "Qualification control PR must already be merged and closed.",
...
def test_exact_reviewed_runner_and_dependency_blobs_are_pinned() -> None:
    text = _text()
    for sha in (
...
def test_preserved_pr119_artifact_identity_and_archive_bytes_are_exact() -> None:
    text = _text()
    required = (
...
def test_exact_offline_runner_emits_receipt_and_projection() -> None:
    text = _text()
    ...
    assert "--output qualification-receipt.json" in text
    assert "--projection-output utc-native-feature-projection.ndjson" in text
    assert "network_performed_by_qualification_runner': False" in text


def test_success_gate_proves_exact_21326_row_research_result_only() -> None:
    text = _text()
    required = (
...
def test_failure_evidence_is_preserved_and_never_auto_replayed() -> None:
    text = _text()
    assert "if: always()" in text
    assert "qualification-artifact/" in text
    assert "if-no-files-found: error" in text
    assert "retention-days: 30" in text
    assert "EXECUTION_COMPLETED_EXACT_PR119_UTC_NATIVE_FEATURE_PROJECTION_EVIDENCE_PRESERVED" in text
