...
def test_current_entrypoint_does_not_accept_raw_research_xg_or_caller_lines():
    ...
    assert "weh_feature_row" not in params
...
def test_total_goals_multiple_exact_lines_are_all_preserved():
    scan = adapter.scan_fixture_all_markets(
        fixture_identity="FOTMOB:2b",
        research_xg=_research_xg(),
        total_goals_lines=(1.5, 2.5, 3.5),
    )
    tg = next(a for a in scan.market_assessments if a.market_id is MarketId.TOTAL_GOALS)
    assert tg.disposition is adapter.ShadowDisposition.ANALYTICAL_READY
    lines = sorted({e.line for e in tg.event_probabilities if e.line is not None})
...
def test_unknown_forged_provider_status_fails_closed():
    scan = adapter.scan_fixture_all_markets(
        fixture_identity="FOTMOB:502",
        research_xg=_research_xg(),
        provider_semantic_by_market={
            MarketId.MATCH_RESULT: "FORGED_PROVIDER_SUPPORTED",
            MarketId.BTTS: "CURRENT_PROVIDER_UNAVAILABLE_UNPROVEN",
        },
    )
    mr = next(a for a in scan.market_assessments if a.market_id is MarketId.MATCH_RESULT)
    assert mr.disposition is adapter.ShadowDisposition.ANALYTICAL_READY_PROVIDER_BLOCKED
    btts = next(a for a in scan.market_assessments if a.market_id is MarketId.BTTS)
    assert btts.disposition is adapter.ShadowDisposition.ANALYTICAL_READY_PROVIDER_BLOCKED


def test_supported_provider_status_keeps_analytical_ready():
    scan = adapter.scan_fixture_all_markets(
        fixture_identity="FOTMOB:503",
        research_xg=_research_xg(),
        provider_semantic_by_market={
            MarketId.MATCH_RESULT: ProviderSemanticStatus.SUPPORTED.value,
        },
    )
    row = next(a for a in scan.market_assessments if a.market_id is MarketId.MATCH_RESULT)
    assert row.disposition is adapter.ShadowDisposition.ANALYTICAL_READY
    assert row.provider_semantic_status == "SUPPORTED"


def test_current_entrypoint_rejects_caller_minted_weh_features():
    params = inspect.signature(adapter.scan_current_fixture_all_markets).parameters
    assert "weh_feature_row" not in params


def test_math_helper_still_accepts_reviewed_style_weh_features():
    params = inspect.signature(adapter.scan_fixture_all_markets).parameters
    assert "weh_feature_row" in params
    features = {name: 0.0 for name in PRE_MATCH_FEATURE_NAMES}
    scan = adapter.scan_fixture_all_markets(
        fixture_identity="FOTMOB:WEH-MATH",
        research_xg=_research_xg(),
        weh_feature_row=features,
    )
    for market_id in (MarketId.HOME_WIN_EITHER_HALF, MarketId.AWAY_WIN_EITHER_HALF):
        ...
        assert row.disposition in {
            adapter.ShadowDisposition.ANALYTICAL_READY,
            adapter.ShadowDisposition.ANALYTICAL_READY_PROVIDER_BLOCKED,
            adapter.ShadowDisposition.SPECIALIST_FEATURES_MISSING,
        }
        assert row.pricing_authority.value == "NOT_AUTHORIZED"
        assert row.selection_authority.value == "NOT_AUTHORIZED"


def test_provider_support_does_not_override_missing_weh_features():
    scan = adapter.scan_fixture_all_markets(
        fixture_identity="FOTMOB:WEH-PROVIDER",
        research_xg=_research_xg(),
        weh_feature_row=None,
        provider_semantic_by_market={
            MarketId.HOME_WIN_EITHER_HALF: ProviderSemanticStatus.SUPPORTED.value,
            MarketId.AWAY_WIN_EITHER_HALF: ProviderSemanticStatus.SUPPORTED.value,
        },
    )
    for market_id in (MarketId.HOME_WIN_EITHER_HALF, MarketId.AWAY_WIN_EITHER_HALF):
        row = next(a for a in scan.market_assessments if a.market_id is market_id)
