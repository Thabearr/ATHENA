...
def _manifest_bytes(payload: dict | None = None) -> bytes:
    return _canonical(payload or _payload())


def test_exact_persisted_bytes_verify_without_parsing_raw_body() -> None:
    manifest = _manifest_bytes()
    verified = verify_persisted_match_details_evidence(manifest_bytes=manifest, raw_bytes=RAW)
    ...
    assert sha256_persisted_match_details_evidence_receipt(verified) == hashlib.sha256(canonical).hexdigest()


def test_manifest_must_be_exact_canonical_bytes() -> None:
    with pytest.raises(FotMobReviewedMatchDetailsPersistedEvidenceError, match="exact canonical PR #50"):
        verify_persisted_match_details_evidence(
            manifest_bytes=_manifest_bytes() + b"\n", raw_bytes=RAW
        )


def test_duplicate_manifest_keys_fail_closed() -> None:
    manifest = b'{"schema_version":1,"schema_version":1}\n'
    with pytest.raises(FotMobReviewedMatchDetailsPersistedEvidenceError, match="duplicate manifest JSON key"):
        verify_persisted_match_details_evidence(manifest_bytes=manifest, raw_bytes=RAW)


def test_embedded_plan_sha_and_size_must_match_exact_canonical_plan() -> None:
    payload = _payload()
    payload["plan_sha256"] = "f" * 64
    with pytest.raises(FotMobReviewedMatchDetailsPersistedEvidenceError, match="plan SHA-256"):
        verify_persisted_match_details_evidence(
    ...
    payload = _payload()
    payload["plan_size"] += 1
    with pytest.raises(FotMobReviewedMatchDetailsPersistedEvidenceError, match="plan size"):
        verify_persisted_match_details_evidence(
            manifest_bytes=_manifest_bytes(payload), raw_bytes=RAW
        )
...
def test_raw_tamper_size_and_hash_fail_closed() -> None:
    with pytest.raises(FotMobReviewedMatchDetailsPersistedEvidenceError, match="size does not match"):
        verify_persisted_match_details_evidence(
            manifest_bytes=_manifest_bytes(), raw_bytes=RAW + b"x"
        )
    payload = _payload()
    payload["raw_sha256"] = "f" * 64
    with pytest.raises(FotMobReviewedMatchDetailsPersistedEvidenceError, match="SHA-256"):
        verify_persisted_match_details_evidence(
            manifest_bytes=_manifest_bytes(payload), raw_bytes=RAW
        )
...
def test_fixture_identity_timing_and_safety_are_fail_closed() -> None:
    ...
    with pytest.raises(FotMobReviewedMatchDetailsPersistedEvidenceError, match="mismatch"):
        verify_persisted_match_details_evidence(manifest_bytes=_manifest_bytes(payload), raw_bytes=RAW)
...
def test_receipt_revalidation_rejects_internally_inconsistent_mutation() -> None:
    verified = verify_persisted_match_details_evidence(manifest_bytes=_manifest_bytes(), raw_bytes=RAW)
    with pytest.raises(FotMobReviewedMatchDetailsPersistedEvidenceError, match="content_length"):
        dataclasses.replace(verified, content_length=verified.raw_size + 1)


def test_network_flag_must_record_real_pr51_acquisition() -> None:
    payload = _payload()
    payload["network_acquisition_performed"] = False
    with pytest.raises(FotMobReviewedMatchDetailsPersistedEvidenceError, match="network acquisition performed"):
        verify_persisted_match_details_evidence(manifest_bytes=_manifest_bytes(payload), raw_bytes=RAW)
