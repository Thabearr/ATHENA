...
@lru_cache(maxsize=1)
def _pr63_helper():
    path = Path(__file__).with_name("test_fotmob_reviewed_match_details_snapshot_candidate_set.py")
    spec = importlib.util.spec_from_file_location("_athena_pr64_pr63_helper", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("could not load PR #63 helper")
