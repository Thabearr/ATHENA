...
FotMob competition identities intersect ATHENA's reviewed competition-review
priority registry. A generic competition name is never enough: country code and
source name must match an explicitly reviewed pair. It grants no candidate
review, fixture admission, model, pricing, selection, accumulator, or BET
authority.
"""
...
SCHEMA_VERSION = 2
DATASET_NAME = "athena-saturday-2026-08-22-fixture-universe-v2"
...
def resolve_fotmob_source_priority(candidate: Any):
    """Resolve review priority only from exact FotMob ccode + whole-name pair."""

    return resolve_source_competition_review_priority(
        getattr(candidate, "source_competition_ccode", None),
        getattr(candidate, "source_competition_name", None),
    )
...
def _candidate_record(candidate: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "competition_review_name": entry.canonical_name if entry else None,
        "competition_review_rank": (
            entry.rank if entry else UNPRIORITIZED_COMPETITION_RANK
        ),
        "competition_review_tier": entry.tier if entry else None,
        "competition_review_kind": entry.kind.value if entry else None,
        "competition_review_source_identity_match": source_identity_match,
        "competition_review_source_identity_basis": (
...
def build_saturday_fixture_universe(bundle: FotMobFixtureCandidateBundle) -> dict[str, Any]:
    ...
    prioritized_competition_counts: dict[str, int] = {}
    ...
    for item in records:
        ...
        if item["competition_review_source_identity_match"]:
            _increment(
                prioritized_competition_counts,
                item["competition_review_name"],
            )
        else:
            _increment(unprioritized_source_competition_counts, source_key)

    prioritized_count = sum(prioritized_competition_counts.values())
    return {
        ...
        "competition_review_source_identity_match_count": prioritized_count,
        ...
        "prioritized_competition_counts": dict(
            sorted(prioritized_competition_counts.items())
        ),
