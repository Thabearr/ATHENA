"""Canonical ATHENA Router / coherence policy.

P3.3 publishes this canonical API under the permanent ``domain.market_router``
name. The historical Phase-8/v1 implementation remains separately available as
``domain._historical_market_router_v1``. This module consumes the canonical
PriceAllEvaluation, delegates exact source reconstruction and the frozen
value/context gates to the reviewed current Router implementation, and promotes
one routing contract with comparable prediction confidence, quote-independent
tie-breaking, logical coherence diagnostics, and counterfactual recording.
...
from domain import _market_router_current_provider as _v3
